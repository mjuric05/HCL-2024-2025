...
export default function CarCategoriesLargePage() {
    ...
    useEffect(() => {
        async function fetchCars() {
            try {
                ...
                // Filter for large cars
                const largeCars = fetchedCars.filter(car => car.size === 'large')
                setCars(largeCars)
                setIsLoading(false)
            } catch (error) {
    ...
    if (isLoading) {
        return <div className="text-center mt-40">Loading Large Cars...</div>
    }
    ...
    return (
        <main className="flex min-h-screen flex-col items-center p-4 pt-24 md:p-10 md:pt-28">
            <h1 className="text-3xl md:text-4xl font-semibold mt-4 text-center text-[#9747FF]">Large Cars</h1>
            <div className="w-full max-w-screen-lg mx-auto mt-8">
                {cars.length > 0 ? (
                    ...
                    <div className="text-center text-gray-500 mt-8">
                        No large cars available at the moment.
                    </div>
                )}
            </div>
        </main>
